...
export async function UpdateOrCreateBrifingDoc(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        const brifingDoc = await generateBrifingDoc(llm, splittingDoc)
        // ensure summary is a string (generateSummary may return string | null)
        const summary: string = brifingDoc ?? ''
        await docRepo.updateSummary({ userId, noteId, summary })
        return res.status(200).json({ message: "BrifingDoc generated and saved successfully", summary })
