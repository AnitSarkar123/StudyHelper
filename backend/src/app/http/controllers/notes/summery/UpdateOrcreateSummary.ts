...
const summery = await generateSummary(llm, splittingDoc)
// ensure summary is a string (generateSummary may return string | null)
const summary: string = summery ?? ''
await docRepo.updateSummary({userId, noteId, summary})
return res.status(200).json({ message: "Summary generated and saved successfully", summary })
